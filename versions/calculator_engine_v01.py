...
class EquilibriumUtils:
    ...
    def solve_free_metal(
        ...
        for a single logK approach (no separate MHL).
        """
        ...
        return metals_free, chelators_free
...
if __name__ == "__main__":
    ...
    temp_celsius = 23.0
    chelator_conc_mM = 5.0
    total_metal_mM = 1.0

    # 3) Get metal constants
    ...
    # 5) Compute [H+], log_f standard/exp
    ...
    # 6) Adjust the metal–ligand logK for ionic strength & temperature
    g_factor_metal = EquilibriumUtils.calc_charge_factor_for_metal_binding(4, 2)
    ...
    # 7) Protonation steps => partial fraction
    ...
    # 8) Combine metal–ligand logK with chelator protonation => net logK
    ...
    # 9) Build arrays for the old solver (single adjusted logK)
    metals_total = [total_metal_mM / 1000.0]
    chelators_total = [chelator_conc_mM / 1000.0]
    adjusted_logk = [[net_log10_k]]   # shape 1x1
    metal_stoich = [[metal_stoichiometry]]
    ligand_stoich= [[ligand_stoichiometry]]

    # 10) Solve free concentrations
    metals_free, chelators_free = EquilibriumUtils.solve_free_metal(
        metals_total,
        chelators_total,
        adjusted_logk,
        metal_stoich,
    ...
    free_ca = metals_free[0]
    free_egta= chelators_free[0]
    kd_val = 1.0 / apparent_binding_constant

    # 11) Print results
    if metal_stoichiometry > 1 or ligand_stoichiometry > 1:
        print(f"Stoichiometry = {metal_stoichiometry}M : {ligand_stoichiometry}L")

    print(f"Free metal  = {free_ca:.6g} M")
    print(f"Free ligand = {free_egta:.6g} M")
    print(f"Kd          = {kd_val:.6g} M")
