...
class Calculator:
    ...
    def standard_output(
        ...
        """Compute results for the standard output of free metals and chelator """
        ...
        for idx, metal in enumerate(metals):
            species_data.append({
                "name": metal["name"],
                "free": metals_free[idx],
                "total": metal["concentration"]
            })
        for idx, chelator in enumerate(chelators):
            species_data.append({
                "name": chelator["name"],
                "free": chelators_free[idx],
                "total": chelator["concentration"]
            })

        # Prepare Kd values
        complex_data = [
            {
                "complex": f"{kd['metal']}-{kd['chelator']}",
                "Kd": kd["Kd"],
                "low_limit": kd["Kd"] / math.log(0.5),
                "high_limit": kd["Kd"] * math.log(0.5)
            }
            for kd in kd_values
        ]
    ...
    def standard_output(
        ...
        """Compute results for the standard output of free metals and chelator and return them as a dictionary"""
        results = Calculator.mainblock(
            const_utils=const_utils,
            chelators=chelators,
            metals=metals,
